...
function RAMbou() {
	if (RAM.length > (bmodel ? 4096 : 2048)) {  // 2/4KB max size
		RAM = RAM.subarray(0, (bmodel ? 4096 : 2048));
		console.log("Maximum RAM memory size is "+(bmodel ? 4 : 2)+"KB, memory area reduced");
	}
...
function ramAutoInit() {
	if (!bmodel) {
		RAM.fill(0xFF, 0, 2048);
		RAM.set(RAM_hed, 0x4A);
	}
}
