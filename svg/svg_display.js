...
/* create text lcd segments */
	...
	for(var x = 0; x<textSegs.length; x++) {
		...
		mapping["bit"] = seg["bit"];
		mapping["f"] = createTextField(this.svg, seg.id, seg.x, seg.y, (altDisp ? seg.talt : seg.txt), 9, segmentOn, segmentOff);
...
function createTextField(root, id, x, y, text, size, colorOn, colorOff) {
